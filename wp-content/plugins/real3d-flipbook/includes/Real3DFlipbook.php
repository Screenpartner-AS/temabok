...
class Real3DFlipbook {
	...
	public function enqueue_scripts() {
		...
		wp_register_script("real3d-flipbook", $this->PLUGIN_DIR_URL."js/flipbook.min.js", array('jquery', 'real3d-flipbook-iscroll'),$this->PLUGIN_VERSION);
     	...
     	wp_register_script("r3d-shortcode", $this->PLUGIN_DIR_URL."js/r3d-shortcode.js", array('jquery'),$this->PLUGIN_VERSION);
     	...
     	wp_register_style("r3d-shortcode-css", $this->PLUGIN_DIR_URL."css/r3d-shortcode.css", array(),$this->PLUGIN_VERSION);
     	wp_register_style("r3d-single-css", $this->PLUGIN_DIR_URL."css/r3d-single.css", array(),$this->PLUGIN_VERSION);	
	}
	...
	public function save_post($post_ID, $post, $update){
		// error_log(print_r("save_post", true));
	...
	public function save_post_r3d($post_ID, $post, $update){
		...
		if("auto-draft" == $status && $title){
			...
			if(isset($_POST['id'])){
				...
				update_post_meta($post_ID, 'flipbook_options', $flipbook);
				update_post_meta($post_ID, 'flipbook_id', $flipbook_id);

				foreach ($book_info_keys as $key => $val) {
					if(isset($_POST[$val])){
	...
	public function add_meta_boxes(){
		...
		// add_meta_box( 'r3d_post_meta_box', __( 'Flipbook', 'r3dfb' ), array( $this, 'create_meta_box_main' ), 'r3d', 'normal', 'high' );

		add_meta_box( 'r3d_post_meta_box_shortcode', __( 'Shortcode', 'r3dfb' ), array( $this, 'create_meta_box_shortcode' ), 'r3d', 'side', 'high' );

		add_meta_box( 'r3d_post_meta_box_help', __( 'Help', 'r3dfb' ), array( $this, 'create_meta_box_help' ), 'r3d', 'side', 'low' );
	...
	public function create_meta_box_help( $post ){
		...
		if ( $current_screen->post_type == 'r3d' ) {
		  ...
		  <ul>
		    <li>
		      <a class="video-tutorial" href="https://www.youtube.com/watch?v=vPXKZpQb5n0#t=40s" target="_blank"><span class="dashicons dashicons-video-alt3"></span> Video Tutorials</a>
		    </li>
		    <li>
		      <a class="video-tutorial" href="
		  https://dearflip.com/docs/dearflip-wordpress/getting-started/" target="_blank"><span class="dashicons dashicons-book"></span> Documentation</a>
		    </li>
		  </ul>
		  <?php
		}
	...
	// public function create_meta_box_main( $post ){
	...
	public function on_shortcode_r3d($atts, $content=null) {
       ...
       $output = '<div class="r3d_shortcode" style="display: none;"><div class="header-bottom pb--10">
				<div class="container">
					<div class="row align-items-center">
						<div class="col-lg-3">
							<nav class="category-nav">
								<div>
									<a href="javascript:void(0)" class="category-trigger"><i class="fa fa-bars"></i>Catégories</a>
		...
		foreach ($categories as $category) {
	         ...
	         if(count($subcategories) > 0) {
	         	...
	         	$output .= '<li class="cat-item has-children"><a href="#"">'.$category->name .'</a>';

	         	$output .= '<ul class="sub-menu">';

	         	foreach ($subcategories as $subcategory) {
	         		$output .= '<li><a href="#"">'.$subcategory->name .'</a></li>';
	         	...
	         	$output .= '<li class="cat-item"><a href="#"">'.$category->name .'</a></li>';
	         }
	     }
						...
						<div class="col-lg-5">
							<div class="header-search-block">
								...
								<button style="background-color:#27ae61;">Rechercher</button>
							</div>
						</div>
					</div>
				</div>
		...
		if (!wp_script_is( 'r3d-shortcode', 'enqueued' )) {
	     	wp_enqueue_script("r3d-shortcode");
	     }

	     $variables = array(
	        'ajaxurl' => admin_url( 'admin-ajax.php' )
	    );
	    wp_localize_script('r3d-shortcode', "r3d", $variables);

	    if (!wp_style_is( 'r3d-shortcode-css', 'enqueued' )) {
	     	wp_enqueue_style("r3d-shortcode-css");
	     }

	     if (!wp_style_is( 'r3d-main-css', 'enqueued' )) {
	     	wp_enqueue_style("r3d-main-css");
	     }

	     if (!wp_style_is( 'r3d-plugins-css', 'enqueued' )) {
	     	wp_enqueue_style("r3d-plugins-css");
	     }
